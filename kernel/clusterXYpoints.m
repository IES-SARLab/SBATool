...
%% Usage
%
% [clustersCentroids,clustersGeoMedians,clustersXY] = clusterXYpoints(XY,maxdist,minClusterSize,method,mergeflag,[plotfig]);
%
%
%% Description of the variables:
%
% XY             : matrix with 2 cols 
%                  - column 1: X values
%                  - column 2: Y values
%
...
%
% plotfig        : "1" to plot QC figures; default is "0"
%
%
%
%% Author
...
%
%% Code
function [clustersCentroids,clustersGeoMedians,clustersXY] = clusterXYpoints(XY,maxdist,minClusterSize,method,mergeflag,varargin)
...
if numel(varargin)>0; plotfig=varargin{1}; else; plotfig=0; end
if numel(varargin)>1; qcfile=varargin{2}; end
...
if exist(qcfile,'var')
    logging(qcfile,sprintf('Number of clusters: %lu\n',clusterCount)); % display the number of clusters
    logging(qcfile,sprintf('Size of smallest cluster: %lu\n',clusterMinSize)); % display the number of clusters
    logging(qcfile,sprintf('Size of largest cluster: %lu\n',clusterMaxSize)); % display the number of clusters
    logging(qcfile,sprintf('Mean cluster size: %f\n',clusterMeanSize)); % display the number of clusters
    logging(qcfile,sprintf('Median cluster size: %lu\n',clusterMedianSize)); % display the number of clusters
    logging(qcfile,sprintf('Number of points that are not part of any cluster: %lu\n',singlepointCount)); % display the number of single points
else
