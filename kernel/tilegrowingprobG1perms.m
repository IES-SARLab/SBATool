function JtileMultiPerms = tilegrowingprobG1perms(tmpImg,tmpxtilevec,tmpytilevec,thresh,indg,fulltsizes)
%function JtileMultiPerms = tilegrowingprobG1perms(tmpImg,xtilevec,ytilevec,thresh,indg)
...
elseif numel(indg)>3000
    error(sprintf('ERROR: number of patch %d (>3000); need to revise threshG1',numel(indg))); 
end
        ...
        %try    
           ...
           neg_list = [];
       	   ...
       	   while ( ( AD0>=thresh(1)  ) && ...
       	       ...
       	       % Check if new neighbour is inside or outside the image
               ...
               indn = sub2ind(fulltsizes,yn,xn);
       	       [~,ia,ib] = intersect(indn,indg);
       	       ins = (Jtile(ib)==0);
               if sum(ins)>0
                   xn  = xn(ia(ins));
                   yn  = yn(ia(ins));
                   seqt = ib(ins); % now seqt is a subset of seq
                   Jtile(seqt)=1;
                   %display(sprintf('Pert %d, grow %d, #Neigb %d',seq(1),reg_size,numel(seqt)));
                   % Add new neighbors pixels
       	           clear G3p1 AD BC AD SR AS NIA
                   parfor j=1:numel(seqt)
                   %for j=1:numel(seqt)
                       [P,M] = getStatG3( [sumImg tmpImg{seqt(j)}], 0, 0, Gtype ); 
                       G3p1{j}=P.G3p1;
                       AD(j)=M.AD1;
                       BC(j)=M.BC;
                       SR(j)=M.SR1;
       	       ...
       	       sumImg = [sumImg cell2mat(tmpImg(neg_list(indQualify,3)))];  % combined statistics of the seed tile and a neighbor tile
