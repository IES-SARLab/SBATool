function  outDict  = xml2dict(input)
...
outDict = parseChildDict(xDoc);
    
end

% ----- Local function parseChildStruct -----
function  dict = parseChildDict(theNode)
...
if hasChildNodes(theNode)
    ...
    for ii = 1:numChildNodes

        theChild = item(childNodes,ii-1);
        [text, name, attr, childs, textflag] = getNodeData(theChild);

        for jj = 1:numel(childs.property)
    
            keys{end+1}   = childs.property{jj}.Attributes.name;
            values{end+1} = childs.property{jj}.value.Text;
        
        end


        for jj = 1:numel(childs.component)

            keys_com = {};
            values_com = {};

            for kk = 1:numel(childs.component{jj}.property)
                
                keys_com{end+1}  = childs.component{jj}.property{kk}.Attributes.name;
                values_com{end+1}= childs.component{jj}.property{kk}.value.Text;

            end

            keys{end+1} = childs.component{jj}.Attributes.name;
            values{end+1} = containers.Map(keys_com,values_com);
